...
| `src/gpu_placement.py` | **Single-GPU placement policy.** Keep a model on one card unless it cannot fit — a split buys capacity, not speed, and costs a cross-device hop per token (94 → 170 tok/s for llama.cpp here). `gpus()` reads nvidia-smi; `plan(required_gib, mode)` pins to the emptiest card that fits with headroom and otherwise leaves the model split; `cuda_env(index)` emits the child env. Consumed by `ollama_server` and `rerank`. Optional and graceful: no nvidia-smi, one card, or a model too big all yield an unpinned plan and nothing changes. Two traps live in its docstring — Ollama needs `OLLAMA_VULKAN=0` or a CUDA-hidden card reappears via Vulkan, and `CUDA_DEVICE_ORDER=PCI_BUS_ID` or index N is not nvidia-smi's GPU N. See [docs/gpu.md](docs/gpu.md). | Done |
| `src/ollama_server.py` | **The GPU-pinned Ollama daemon.** The system daemon on `:11434` runs as `User=ollama` with `OLLAMA_NUM_PARALLEL=4` and no pin, and the box's other apps depend on it — so this starts a *second* `ollama serve` on `OLLAMA_PIN_PORT` against the **same model store** (`find_models_dir()`: most manifests, not first-that-exists — `~/.ollama/models` is empty here), pinned via `gpu_placement`, with `OLLAMA_NUM_PARALLEL=1`. `host()`/`status()` resolve once per process; `required_gib()` sums the *distinct* models every role is configured with (Ollama keeps several resident at once). **Fails open on everything** — remote `OLLAMA_HOST`, pinning off, model too big, no binary, port never comes up ⇒ the configured host unchanged. An already-serving pinned port is *adopted*, not duplicated, so a hard kill strands at most one daemon; `atexit` reaps one we own. See [docs/gpu.md](docs/gpu.md). | Done |
