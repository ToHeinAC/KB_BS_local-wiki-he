...
| `src/prompts.py` | All LLM prompt constants including `WIKI_SEARCH_DESCRIPTION`, `WIKI_READ_DESCRIPTION`, `RESEARCHER_INSTRUCTIONS`, `INGEST_PROMPT`, `GENERATE_QUESTIONS_PROMPT`, `DEEP_CALCULATE_DESCRIPTION`, etc. | Done |
| `src/tools.py` | Deep-researcher tools: `wiki_search`/`wiki_read`, `tavily_search`, `fetch_webpage_content`, `think_tool`, `submit_final_answer`, `deep_calculate`. Deep-chat tools: `raw_search` (BM25 via `lex_index.query()`), `raw_read` (paginated), `submit_chat_answer` (halved gates), `deep_calculate`. | Done |
