...
| `src/wiki_engine.py` | `init_wiki`, three-stage `ingest_begin` / `ingest_piece` / `ingest_end` (source-scoped chunker+qa_gen+select-affected → per-piece LLM synthesis with auto-merged `sources:` frontmatter → single `lex_index.build()`), back-compat `ingest()` wrapper, `rebuild_lex_index`, `query`/`query_with_sources`, `file_answer`, `lint`, `build_link_graph`, `build_typed_graph` (page + source nodes, typed edges), `find_orphans`, `resolve_contradiction`, `list_pages`, `read_page`, `read_page_parsed`, `stats`, `search_wiki`, `get_wiki_tree` | Done |
