...
| `src/auth.py` | Local user store at `data/users.json` (gitignored). bcrypt password hashes; per-user DB allowlist + `is_admin`. `verify`, `add_user`, `delete_user`, `set_user_dbs`, `change_password`, `user_dbs`, `is_admin`. `ensure_seeded()` creates default admin `T. Hein`/`k-wiki` on first run. | Done |
