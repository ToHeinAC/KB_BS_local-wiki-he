...
| `src/tools.py` | Deep-researcher tools: `wiki_search` (appends `[Wiki linked N]` results — 1-hop `related:` neighbours of top hits via `wiki_engine.linked_pages`; gated by `WIKI_LINK_EXPANSION`/`WIKI_LINK_SEEDS`/`WIKI_LINK_MAX`)/`wiki_read`, `tavily_search`, `fetch_webpage_content`, `think_tool`, `submit_final_answer`, `evaluate_condition`. Deep-chat tools: `raw_search` (BM25 via `lex_index.query()`), `raw_read` (section-suffixed reads resolve to that section's chunk text — legal `§` + markdown headings; byte `offset` fallback for section-less files), `submit_chat_answer` (halved gates), `evaluate_condition`. | Done |
