...
| **Maintenance** | Wiki stats, **Search index** (per-scope row counts from `lex_index.index_health()`, a warning when the DB has no `chunks.sqlite`, and a maintainer-only *Rebuild search index* button → `wiki_engine.rebuild_lex_index()`), link-graph health (orphan check), lint, activity log (all readers). *Maintainer-only:* **Delete Source** (selectbox of registered sources + irreversibility warning + confirmation checkbox → cascading delete of raw file, chunks, QA rows, wiki pages, then index rebuild) and **Reset all data**. *Admin-only:* Users / Databases management (create DB + assign maintainers, edit per-user access + maintained DBs). | 3.9.6 |
