...
* **The controls are ranked, not laid out side by side.** Graph|Tree is a two-option segmented control (it was a radio, which read as a form field); under Graph, *Galaxy / Ranked / Clusters* takes the full width on its own row (`width="stretch"` + the `.st-key-graph_layout` flex rule), because it is the control most visits touch. **Connections** and **Style Options** refine what is already drawn, so they sit in a collapsed `Advanced` expander. They still instantiate on every run — an expander renders its body whether open or shut — so nothing about their session-state persistence changes.
