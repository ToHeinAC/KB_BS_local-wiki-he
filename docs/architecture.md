...
| `src/ontology_evolution.py` | **Evolving the ontology (pure).** Editor tables ↔ state, cue tester, re-classify plan (rule facts follow cues; user decisions kept), validation of model-suggested classes. See [ontology.md](ontology.md) §Evolution. | Phase 7 |
| `src/ontology_export.py` | **Standard exports (pure).** Schema as SKOS Turtle, facts as schema.org/ELI JSON-LD (`scripts/export_ontology.py`, Overview download buttons). | Phase 7 |
