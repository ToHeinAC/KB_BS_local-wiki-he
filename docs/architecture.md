...
| `src/ontology_store.py` | **Ontology I/O adapter.** Shared modules in `ontology/`, binding `data/<DB>/ontology.yaml`, append-only ledger `data/<DB>/ontology/assertions.jsonl` (flock, sequential ids, torn-line repair); `load()` fails open to `(None, errors)`; `retract_subject()` for `delete_source`. | Phase 1 |
