...
- **`src/ollama_client.py`** is the *only* place that imports `ollama`. Exposes `generate()`, `chat()`, `is_available()`, plus `ocr(model_id, prompt, image_b64)` (vision call with one base64 image) and `rewrite(model_id, prompt)` (text reformat with a per-call model id) used by `md_convert.py`.
