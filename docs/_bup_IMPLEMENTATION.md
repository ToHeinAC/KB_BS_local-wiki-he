...
| `src/page_lang.py` (one language per wiki page) | **Done** |
| `src/metadata_extract.py` (regex effective-date detection) | **Done** |
| Test suite | **Done** (527 tests, 524 passing — see §5 and [`docs/tests.md`](docs/tests.md)) |
...
| GPU placement (single-card pinning, the managed Ollama daemon) | [`docs/gpu.md`](docs/gpu.md) | §2.3, §5 |
...
| Change log archives | [`docs/changelog-archive-2.md`](docs/changelog-archive-2.md) (2026-07-08 … 07-23), [`docs/changelog-archive.md`](docs/changelog-archive.md) (older) | — |
...
| `src/rerank.py` | **Cross-encoder reranker (Stage D).** `bge-reranker-v2-m3` GGUF run **in-process** via `llama-cpp-python`, pinned to one GPU; position-aware blend with the fused score, `rerank_score` feeds Stage E. **Fails open on everything.** Ollama cannot host it and `Llama.rank` is not exposed — see AGENTS.md §5.3 and [docs/retrieval.md](docs/retrieval.md). | Done |
| `src/calibrate.py` | **Calibrated abstention (Stage E).** `assess(hits, db)` abstains only when a per-DB τ (`index/calibration.json`, from `scripts/calibrate_abstention.py`) exists **and** the best `rerank_score` is below it; uncalibrated ⇒ never abstains. Deep answer paths only. See [docs/retrieval.md](docs/retrieval.md). | Done |
...
- **Test suite: 527 tests (524 passing; the 3 `test_file_processor` truncation tests fail), no hard cap** — the source-of-truth count. PRD §4.5's original 100-cap was superseded 2026-05 (new modules with verifiable behaviour are exempt); keep the suite lean and high-signal, no low-value proliferation.
...
| `OLLAMA_PIN_GPU` | `auto` | GPU placement for Ollama. `auto` starts an app-managed `ollama serve` pinned to the emptiest card that fits the configured models; a bare index forces that card; `off` uses `OLLAMA_HOST` as-is. Every failure falls back to `OLLAMA_HOST`. See [docs/gpu.md](docs/gpu.md). |
