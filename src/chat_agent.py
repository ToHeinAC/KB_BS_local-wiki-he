...
def _build_llm() -> Runnable[LanguageModelInput, BaseMessage]:
    ...
    ).bind_tools(tool_module.CHAT_TOOLS)  # pyright: ignore[reportUnknownMemberType]  # bare Callable
...
def _build_graph(
    ...
    g.add_node("tools", ToolNode(tool_module.CHAT_TOOLS))  # pyright: ignore[reportUnknownMemberType]
