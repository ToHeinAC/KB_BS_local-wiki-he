...
CODE_DIRS = ("src", "tests", "scripts", ".claude/hooks")
VENDOR = ROOT / "src" / "vendor"  # third-party source, never hand-edited
...
def test_repo_functions_within_limit() -> None:
    files = [p for d in CODE_DIRS for p in (ROOT / d).rglob("*.py") if VENDOR not in p.parents]
    offenders = {
