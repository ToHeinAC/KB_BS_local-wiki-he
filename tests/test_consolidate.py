...
def test_collapses_near_duplicate_concepts(wiki_dir):
    ...
    _write(
        wiki_dir, "concept-dense-llms.md", "Dense LLMs", "concept", "## Key facts\n- y\nFrom two"
    )
    wiki_engine.consolidate(dry_run=False)
...
def test_remaps_related_and_cleans_citations(wiki_dir):
    ...
    _write(
        wiki_dir, "concept-other.md", "Other", "concept", "Body", related=["concept-dense-llms.md"]
    )
    wiki_engine.consolidate(dry_run=False)
