...
def test_canonical_slug_tokens_fold_plural_and_prefix():
    assert w._canonical_slug_tokens("dense-llms") == w._canonical_slug_tokens(
...
def test_canonical_slug_tokens_subset_relations():
    ...
    assert w._canonical_slug_tokens("mixture-of-experts-moe") <= w._canonical_slug_tokens(
...
def _reg(entries):
    return {
        fn: {"type": t, "tokens": w._canonical_slug_tokens(title), "terms": frozenset(terms)}
...
def test_route_exact_topic_match():
    ...
    target = w._route_page(
        "concept",
        w._canonical_slug_tokens("Dense LLMs"),
        frozenset(["dens", "llm"]),
        reg,
...
def test_route_subset_requires_term_overlap():
    ...
    target = w._route_page(
        "concept",
        w._canonical_slug_tokens("Edge AI Inference"),
        frozenset(["edge", "ai"]),
        reg,
        "concept-edge-ai-inference.md",
    )
...
def test_route_no_match_for_unrelated():
    ...
    target = w._route_page(
        "concept",
        w._canonical_slug_tokens("Densing Law"),
        frozenset(["dens", "law"]),
        reg,
        "concept-densing-law.md",
    )
...
def test_route_never_crosses_type():
    ...
    target = w._route_page(
        "concept",
        w._canonical_slug_tokens("Dense LLM"),
        frozenset(["dens", "llm"]),
        reg,
        "concept-dense-llm.md",
    )
