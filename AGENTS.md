...
- **Ontology writes go through `ontology_store` plans.** Shared modules `ontology/*.yaml` change via git only; a DB's binding and facts change only through `prepare_*` → `apply` (validated, maintainer-checked, revisioned, logged) or, for code, `ledger rows + record_change(via)`. The ledger is append-only. A revision is recorded only when the canonical hash moves. See [docs/ontology.md](docs/ontology.md).
